...
import { Project, ProjectsStatus } from './entities/project.entity';
...
export class ProjectsService {
  ...
  create(createProjectDto: CreateProjectDto) {
    ...
    if (createProjectDto.started_at) {
      project.status = ProjectsStatus.Active;
    }
  ...
  findOne(id: string) {
    return this, this.projectRepo.findOneOrFail({ where: { id } });
  }
  ...
  async update(id: string, updateProjectDto: UpdateProjectDto) {
    ...
    if (updateProjectDto.started_at) {
      ...
      if (project.status === ProjectsStatus.Completed) {
        throw new Error('Cannot start completed project');
      }

      if (project.status === ProjectsStatus.Cancelled) {
    ...
    if (updateProjectDto.cancelled_at) {
      ...
      project.status = ProjectsStatus.Cancelled;
    }
